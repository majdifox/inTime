...
class EnhancePassengerRideFeatures extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        ...
        // For PostgreSQL, handle vehicle types as string instead of enum
        Schema::table('vehicles', function (Blueprint $table) {
            // Check if type column exists but isn't string type
            if (Schema::hasColumn('vehicles', 'type') && Schema::getColumnType('vehicles', 'type') !== 'string') {
                // Create a temporary column
                $table->string('type_new')->nullable();
                
                // Copy data to the new column
                DB::statement("UPDATE vehicles SET type_new = type::text");
