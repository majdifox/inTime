...
<head>
    ...
    <title>@if($driver && $driver->user) {{ $driver->user->name }} - Driver Profile @else Driver Profile @endif | inTime</title>
    ...
    <main class="container mx-auto px-4 py-8">
        @if(!$driver)
            <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
                ...
                <span class="block sm:inline">Driver not found.</span>
            </div>
        @else
                    ...
                    <div class="bg-white rounded-lg shadow-md p-6">
                        <div class="flex items-center space-x-4 mb-6">
                            <div class="h-20 w-20 rounded-full bg-gray-300 overflow-hidden flex-shrink-0">
                                @if($driver->user && $driver->user->profile_picture)
                                    <img src="{{ asset('storage/' . $driver->user->profile_picture) }}" alt="{{ $driver->user ? $driver->user->name : 'Driver' }}" class="h-full w-full object-cover">
                                @else
                                    <div class="h-full w-full flex items-center justify-center bg-gray-400 text-white text-2xl font-bold">
                                        {{ $driver->user ? strtoupper(substr($driver->user->name, 0, 1)) : 'D' }}
                                    </div>
                                @endif
                            </div>
                            <div>
                                <h1 class="text-xl font-bold">{{ $driver->user ? $driver->user->name : 'Driver' }}</h1>
                                <div class="flex items-center mt-1">
                                    <div class="flex items-center">
                                        <span class="text-lg font-medium mr-1">{{ number_format($driver->rating ?? 5.0, 1) }}</span>
                                ...
                                <p class="text-gray-500 text-sm mt-1">Driver since {{ $driver->created_at ? $driver->created_at->format('F Y') : 'N/A' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="text-gray-600">
                                    @if($driver->bio)
                                        {{ $driver->bio }}
                                    @else
                                        Professional driver with {{ $driver->completed_rides ?? 0 }} completed rides.
                                    @endif
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p class="font-semibold">{{ $driver->completed_rides ?? 0 }}</p>
                                    </div>
                                    <div>
                            ...
                            <div>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    @if($driver->women_only_driver && $driver->user && $driver->user->gender === 'female')
                                    ...
                                    <!-- Experience badge -->
                                    @if(($driver->completed_rides ?? 0) > 500)
        ...
        @if(count($reviews) > 0)
            <div class="space-y-6">
                @foreach($reviews as $review)
                    @php
                        // Validate the relationship chain is intact 
                        $valid = $review && $review->passenger && $review->passenger->user;
                        
                        // Get passenger name or fallback
                        $passengerName = $valid ? $review->passenger->user->name : 'Passenger';
                        
                        // Get first letter for avatar
                        $initial = $valid ? substr($passengerName, 0, 1) : 'P';
                        
                        // Get profile picture if available
                        $hasProfilePic = $valid && $review->passenger->user->profile_picture;
                    @endphp
                    <div class="border-b border-gray-200 pb-4 {{ !$loop->last ? 'mb-4' : '' }}">
                        <div class="flex items-start">
                            <div class="h-10 w-10 rounded-full bg-gray-300 overflow-hidden mr-3 flex-shrink-0">
                                @if($hasProfilePic)
                                    <img src="{{ asset('storage/' . $review->passenger->user->profile_picture) }}" alt="{{ $passengerName }}" class="h-full w-full object-cover">
                                @else
                            ...
                            <div class="flex-1">
                                <div class="flex justify-between">
                                    <div>
                                        <p class="font-medium">{{ $passengerName }}</p>
