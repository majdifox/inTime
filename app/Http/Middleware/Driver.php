    ...
    public function handle(Request $request, Closure $next): Response
{
    ...
    // CHANGE HERE: If suspended, redirect to accountDeactivated view
    if ($status === 'suspended') {
        // Allow access to account deactivated page
        if ($request->routeIs('driver.account.suspended')) {
            return $next($request);
        }
        // Redirect to account deactivated page
        return redirect()->route('driver.account.suspended');
    }
    
    // If deleted, show appropriate message
    if ($status === 'deleted') {
        return redirect()->route('home')->with('error', 'Your account has been deleted. Please contact support for assistance.');
    }
