...
public function activeRide()
{
    ...
    // Get active ride OR most recently completed rides
    $activeRide = Ride::with(['driver.user', 'driver.vehicle', 'driver.driverLocation'])
        ->where('passenger_id', $passenger->id)
        ->where(function($query) {
            $query->where(function($q) {
                ...
                // Recently completed ride that needs payment or rating
    ...
    \Log::info('Active ride found', [
        ...
        'is_paid' => $activeRide->is_paid,
        'payment_status' => $activeRide->payment_status,
        'has_dropoff_time' => $activeRide->dropoff_time ? 'yes' : 'no'
    ]);
    
    // If the ride is completed and needs payment, redirect to payment page
    if ($activeRide->ride_status === 'completed' && !$activeRide->is_paid) {
        \Log::info('Redirecting to payment page for ride: ' . $activeRide->id);
        return redirect()->route('passenger.ride.payment', $activeRide->id);
    }
    
    // If the ride is completed, paid but not rated, show the rating view
    if ($activeRide->ride_status === 'completed' && $activeRide->is_paid && !$activeRide->is_reviewed) {
        \Log::info('Redirecting to rating page for ride: ' . $activeRide->id);
        return redirect()->route('passenger.rate.ride', $activeRide->id);
    }
    
    // Otherwise show the active ride view
...
{
    \Log::info('Show payment page called for ride ID: ' . $ride->id);
    ...
    // Log the ride status for debugging
    \Log::info('Ride payment page - ride status', [
        'ride_id' => $ride->id,
        'ride_status' => $ride->ride_status,
        'is_paid' => $ride->is_paid ? 'yes' : 'no',
        'payment_status' => $ride->payment_status,
        'has_dropoff_time' => $ride->dropoff_time ? 'yes' : 'no'
    ]);
    
    // Check if ride is completed
    if ($ride->ride_status !== 'completed') {
        \Log::warning('Attempted to view payment page for incomplete ride', [
            'ride_id' => $ride->id,
            'ride_status' => $ride->ride_status
        ]);
    ...
    if ($ride->is_paid) {
        ...
        \Log::info('Ride already paid, redirecting to rating page', [
            'ride_id' => $ride->id
        ]);
    ...
    \Log::info('Successfully displaying payment page for ride', [
        'ride_id' => $ride->id,
        'saved_cards_count' => $savedCards->count()
    ]);
